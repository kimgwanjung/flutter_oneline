...
abstract class IRowState {
  ...
  void saveAndDeleteRow(int deleteindex);
...
mixin RowState implements IPlutoGridState {
  ...
  void removeCurrentRow() {
    ...
    if (enabledRowGroups) {
      ...
      if (refRows[currentRowIdx!].cells['status']!.value == 'delete') {
        refRows[currentRowIdx!].cells['status']!.value = 'saved';
      } else {
        refRows[currentRowIdx!].cells['status']!.value = 'delete';
        var savedindex = refRows
            .asMap()
            .entries
            .where((element) => element.key != currentRowIdx!)
            .map(
              (e) => e.key,
            );
        print('print value: ${savedindex}');

        for (var x in savedindex) {
          var target = refRows[x].cells['status']!.value = 'saved';

          print('target: $target');
        }
      }
      // refRows.removeAt(currentRowIdx!);
      // print('removeAt Index : $currentRowIdx!');
  ...
  @override
  void saveAndDeleteRow(int deleteindex) {
    print('remove index :$deleteindex');
    refRows.removeAt(deleteindex);
    // resetCurrentState(notify: true);
    notifyListeners(true, saveAndDeleteRow.hashCode);
  }
